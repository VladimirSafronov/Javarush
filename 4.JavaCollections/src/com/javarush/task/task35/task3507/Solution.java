        ...
        if (pathes != null) {
            for (String p : pathes) {
                try {
                    ...
                    String className = p.substring(0, p.length() - 6); //without ".class"
                    Class<?> clazz = loader.loadClass(className); //получаем объект с информацией о классе

                    //можно заменить if (Animal.class.isAssignableFrom(clazz))
                    Class[] interfaces = clazz.getInterfaces(); //получаем интерфейсы, которые реализует класс
...
//        File[] clazzes = Paths.get(pathToAnimals).toFile().listFiles();
//        if (clazzes != null) {
//            for (File f : clazzes) {
//                Class<?> clazz = new MyClassLoader().loadClass(f.getPath());
